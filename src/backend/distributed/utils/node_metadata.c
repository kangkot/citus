 ...
 * returns its data. It also replicates all reference tables to the new node.
 */
Datum
 ...
 * and returns its data. It does not replicate reference tables to the new node,
 * it only adds new node to the pg_dist_node table.
 */
Datum
...
ActivateNode(char *nodeName, int nodePort)
{
	...
	foreach(workerNodeCell, workerNodeList)
	{
		...
		if (strcmp(workerNode->workerName, nodeName) != 0 ||
			workerNode->workerPort != nodePort)
		{
			bool lockAcquired = TryLockNodeId(workerNode->nodeId, ExclusiveLock);
...
master_initialize_node_metadata(PG_FUNCTION_ARGS)
{
	...
	foreach(workerNodeCell, workerNodes)
	{
		...
		AddNodeMetadata(workerNode->workerName, workerNode->workerPort, 0,
						workerNode->workerRack, false, NODE_STATE_ACTIVE,
						&nodeAlreadyExists);
	}
...
SetNodeState(char *nodeName, int32 nodePort, uint32 nodeState)
{
	...
	values[Anum_pg_dist_node_state - 1] = UInt32GetDatum(nodeState);
	isnull[Anum_pg_dist_node_state - 1] = false;
	replace[Anum_pg_dist_node_state - 1] = true;
...
GenerateNodeTuple(WorkerNode *workerNode)
{
	...
	values[Anum_pg_dist_node_state - 1] = UInt32GetDatum(workerNode->workerState);
...
TupleToWorkerNode(TupleDesc tupleDescriptor, HeapTuple heapTuple)
{
	...
	Datum hasMetadata = heap_getattr(heapTuple, Anum_pg_dist_node_hasmetadata,
									 tupleDescriptor, &isNull);
	Datum nodeState = heap_getattr(heapTuple, Anum_pg_dist_node_state, tupleDescriptor,
								   &isNull);
